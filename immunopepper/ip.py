...
def get_build_parser(parser):
    ...
    submodes.add_argument("--libsize-extract",help="This option generates library sizes and gene quantifications and skip neontigen generation. **Note:** See more details about output files in the :ref:`build output section <build_out>`.",action="store_true", required=False, default=False)
    ...
    outputs.add_argument("--output-fasta", help="This option allows to output the foreground peptides in fasta format, in addition to the tabular metadata file. More information about the output files can be found in :ref:`build output section <build_out>` will be generated. If set to False only the kmers will be output.",  action="store_true", required=False, default=False)
    ...
    parameters.add_argument("--disable-process-libsize", help="This option generates the libsize file (More info about the outputs in the :ref:`build output section <build_out>`).", action="store_true", default=False)
...
def get_cancerspecif_parser(parser):
    ...
    inputs.add_argument("--path-cancer-libsize", help="Path for the libsize file of the selected cancer samples. More information about the output format in :ref:`build output section <build_out>`", required=False, default=None)
    inputs.add_argument("--path-normal-libsize", help="Path for the libsize of the selected normal samples. More information about the output format in :ref:`build output section <build_out>`", required=False, default=None)
    ...
    nrf.add_argument("--path-normal-matrix-segm", nargs='+', help="Path to the matrix of segment expression of kmers in normal samples. More information about the output in :ref:`build output section <build_out>`", required=False, default=None)
    nrf.add_argument("--path-normal-matrix-edge", nargs='+', help="Path to the matrix of junction expression of kmers in normal samples. More information about the output in :ref:`build output section <build_out>`", required=False, default=None)
    nrf.add_argument("--n-samples-lim-normal", type=int, help=" Integer number that sets the number of normal samples in which we need to see any expression to exclude a k-mer from the tumor-associated k-mer set. I.e. if the k-mer expression is > 0 in at least n normal samples. More information in :ref:`<cancerspecif>` and :ref:`<cancerspecif_out>`", required=False, default=None)
    nrf.add_argument("--cohort-expr-support-normal", type=float, help="Float that specifies the k-mer expression level (normalized), that if observed in any normal sample, will lead to exclusion for the tumor-associated k-mer set. I.e. if a kmer is found with an expression level higher or equal than this threshold in one or more normal samples it will be considered a normal kmer and excluded as tumor-associated candidate. More information in :ref:`<cancerspecif>` and :ref:`<cancerspecif_out>`", required=False, default=None)
    ...
    more_filters.add_argument("--filterNeojuncCoord", choices=['C', 'N', 'A'], required=False, default='', help=" This argument will filter the neojunctions, and it will retain only the kmers that are generated from neojunctions. These are peptides whose junction coordinates were not part of the annotation files. Selecting this option means that only those kmers with junctionAnnotated = False will be considered for further filtering. This filter is used in the preprocessing (step 1 of cancer and normal pipelines). If 'C' is selected, the filter is only applied to the cancer sample. If 'N' is selected, the filter is only applied to the normal samples. If 'A' is selected, the filter is applied to both cancer and normal samples. More information in :ref:`<cancerspecif>` and :ref:`<cancerspecif_out>`")
    more_filters.add_argument("--filterAnnotatedRF", choices=['C', 'N', 'A'], required=False, default='', help="This argument will only retrieve kmers that were generated from annotated reading frames and discard those that were obtained by reading frame propagation through the graph. By selecting this option, only kmers generated from reading frames present in the annotation are kept. This means that selecting this option means that only those kmers with ReadFrameAnnotated = True will be selected. This filter is used in the preprocessing (step 1 of cancer and normal pipelines). If 'C' is selected, the filter is only applied to the cancer sample. If 'N' is selected, the filter is only applied to the normal samples. If 'A' is selected, the filter is applied to both cancer and normal samples. More information in :ref:`<cancerspecif>` and :ref:`<cancerspecif_out>`")
...
def get_mhcbind_parser(parser):
    ...
    optional = parser.add_argument_group('Optional argument')
    optional.add_argument("--partitioned-tsv", help="The input to this command is the path to the folder containing the partitioned tsv files from `cancerspecif` mode. This corresponds to the files 1 and 2 found in the :ref:`output section <output-tsv-cancerspecif>`. The file partitions should be named part* with kmers in the first column and a tab separation. They can be in gzip format or not. If this parameter is set the tool will directly accept the files from cancerspecif mode as input.", required=False, default=None)
