...
def parse_arguments(argv):
    ...
    outputs.add_argument("--zip", help="specify if compress the output file", action="store_true",default=False)
    ...
    required.add_argument("--kmer_files_list", nargs='+', help="the sample names, can specify more than one sample", required=True, default='')
    ...
    required.add_argument("--kmer_tsv_path", help="the kmer tsv file", required=True, default='')
    ...
    required.add_argument("--output_dir", help="directory to save filtered kmer file", default='')
    general = parser_filter.add_argument_group('MISCELLANEOUS')
    general.add_argument("--verbose", type=int, help="specify the output verbosity", required=False, default=0)

    parser_filter.set_defaults(func=immunopepper_filter)
    if len(argv) < 2:
        parser.print_help()
        sys.exit(1)
