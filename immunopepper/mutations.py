...
def apply_germline_mutation(ref_sequence_file: str, chromosome: str, pos_start: int, pos_end: int,
                            mutation_sub_dict: dict):
    """Apply the given germline mutations to a reference sub-sequence loaded from a file.
    ...
    return {'ref': ref_sequence,
            'background': construct_mut_seq_with_str_concat(ref_sequence, pos_start, pos_end, mutation_sub_dict)}


def construct_mut_seq_with_str_concat(ref_seq, pos_start, pos_end, mut_dict):
    """ Apply the given germline mutations to a reference sequence.
...
def parse_mutation_file(mutation_tag, mutation_file_path, output_dir, heter_code, mut_pickle=False,
...
def get_mutation_mode_from_parser(args, target_sample_list):
    ...
    if mutation_mode == 'somatic_and_germline':
        if somatic_file_path != '' and germline_file_path != '':
            somatic_mutation_dict = parse_mutation_file("somatic", somatic_file_path, output_dir, heter_code,
                                                        mut_pickle, target_sample_list, args.mutation_sample,
                                                        graph_to_somatic_names)
            germline_mutation_dict = parse_mutation_file("germline", germline_file_path, output_dir, heter_code,
                                                         mut_pickle, target_sample_list, args.mutation_sample,
                                                         graph_to_germline_names)
            is_error = False
    ...
    if is_error:
        logging.error(
            "mutations.py: The input mutation file does not match the mutation mode (somatic, germline, somatic_and_germline), please check again")
        sys.exit(1)
    mutation = Mutation(mutation_mode, germline_mutation_dict=germline_mutation_dict,
                        somatic_mutation_dict=somatic_mutation_dict)
    return mutation
...
def get_sub_mutation_tuple(mutation, sample, chrm):
    ...
    submutation = Mutation(mode=mutation.mode, somatic_mutation_dict=somatic_mutation_sub_dict,
                           germline_mutation_dict=germline_mutation_sub_dict)
    return submutation
