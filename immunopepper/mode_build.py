...
def process_gene_batch_background(output_sample, mutation_sample, genes, gene_idxs, all_read_frames, mutation,
                                  countinfo_b, genetable, arg, outbase, filepointer, compression=None, verbose=False):
    if arg.parallel > 1:
...
def process_gene_batch_foreground(output_sample, mutation_sample, output_samples_ids, genes, genes_info, gene_idxs,
                                  ...
                                  mutation, junction_dict, countinfo, genetable, arg, outbase, filepointer, compression,
                                  verbose):
    if arg.parallel > 1:
        batch_name = int(outbase.split('/')[-1].split('_')[-1])
    else:
    ...
    if (arg.parallel == 1) or (not os.path.exists(os.path.join(outbase, "output_sample_IS_SUCCESS"))):
        ...
        if not arg.cross_graph_expr:
            for kmer_length in dict_kmer_foregr:
                save_fg_kmer_dict(dict_kmer_foregr[kmer_length], filepointer, kmer_length, compression, outbase,
                                  verbose)
